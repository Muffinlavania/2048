...
#SCORE SAVING
highscore = 0
if '2048scores.txt' in os.listdir():
  with open('2048scores.txt','r') as k:
    highscore = int(k.read())
else:
  with open('2048scores.txt','w') as k:
    k.write('0')
...
def save():
  global newhigh
  newhigh = False
  with open('2048scores.txt', 'r') as h:
    oldscore = int(h.read())
  
  if oldscore < score:
    newhigh = True
    with open('2048scores.txt','w') as sam:
      sam.write(str(score))
